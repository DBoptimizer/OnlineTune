...
def check_env_setting(workload_name, rds_mode=False):
    ...
    if workload_name == 'tpcc':
        # if not os.environ.get('TPCC_HOME') and not settings['TPCC_HOME']:
        if not os.environ.get('TPCC_HOME'):
            err_msg = 'You should set TPCC_HOME env var before train'
            logger.error(err_msg)
            raise AutotuneError(err_msg)
    elif workload_name == 'sysbench':
        ...
        if not os.environ.get('SYSBENCH_BIN'):
            ...
            logger.error(err_msg)
            raise AutotuneError(err_msg)
    elif workload_name == 'cloudbench_dingding':
        # if not os.environ.get('CLOUDBENCH_BIN') and not settings['CLOUDBENCH_BIN']:
        if not os.environ.get('CLOUDBENCH_BIN'):
            err_msg = 'You should set CLOUDBENCH_BIN before train'
            logger.error(err_msg)
            raise AutotuneError(err_msg)
